...
Matching rows based on equality for all join columns except the last one, and finding the greatest value less than or equal to the left table's value for the last column.

```clj
↪ (asof-join columns left-table right-table)
```

The `asof-join` function takes three arguments: a [:material-vector-line: Vector](../data-types/vector.md) of [:simple-scalar: Symbols](../data-types/symbol.md) representing join columns, the left [:material-table: Table](../data-types/table.md), and the right [:material-table: Table](../data-types/table.md).
